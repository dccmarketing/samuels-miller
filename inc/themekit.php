 ...
 */
function samuels_miller_fonts_url() {

	$return 	= '';
	$families 	= '';
	$fonts[] 	= array( 'font' => 'PT Sans', 'weights' => '400,700', 'translate' => esc_html_x( 'on', 'PT Sans font: on or off', 'samuels-miller' ) );
	$fonts[] 	= array( 'font' => 'Cinzel', 'weights' => '400', 'translate' => esc_html_x( 'on', 'Cinzel font: on or off', 'samuels-miller' ) );
 ...
 */
function samuels_miller_get_featured_images( $postID ) {
 ...
 */
function samuels_miller_get_posts( $post, $params = array() ) {

	$return = '';
	$return = wp_cache_get( 'samuels_miller_' . $post . '_posts', 'samuels_miller_posts' );
	...
	if ( false === $return ) {
		...
		if ( ! is_wp_error( $query ) && $query->have_posts() ) {
			...
			wp_cache_set( 'samuels_miller_' . $post . '_posts', $query, 'samuels_miller_posts', 5 * MINUTE_IN_SECONDS );
 ...
 */
function samuels_miller_get_svg( $svg ) {
 ...
 */
function samuels_miller_get_thumbnail_url( $postID, $size = 'thumbnail' ) {
...
} // samuels_miller_get_thumbnail_url()
 ...
 */
function samuels_miller_shorten_text( $text, $limit = 100, $after = '...' ) {
 ...
 * @return 	mixed 					The SVG code
 */
function samuels_miller_the_svg( $svg ) {

	echo samuels_miller_get_svg( $svg );

} // samuels_miller_the_svg()


function samuels_miller_get_tel_number( $number ) {
	...
	foreach ( $nopes as $nope ) {
		...
		if ( FALSE === strpos( $return, $nope ) ) { continue; }

		$return = str_replace( $nope, '', $number );
 ...
 */
function samuels_miller_add_favicons() { ?>
...
<?php }
add_action( 'wp_head', 'samuels_miller_add_favicons' );
